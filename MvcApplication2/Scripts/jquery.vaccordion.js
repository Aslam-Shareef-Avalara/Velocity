	...
	var cache	= {
		...
		aux		= {
				...
				return $.Deferred(
					function( dfd ) {
						...
						// if it's closed..
                            ...
                            //aslam
							othersHeight = 50;//Math.ceil( ( settings.accordionH - settings.expandedHeight ) / ( settings.visibleSlices - 1 ) );
			...
			slide			: function( dir, $slices, $navNext, $navPrev, settings ) {
				...
				$('.va-title').removeClass("opened");
				$slices.each(function(i) {
					var $slice		= $(this),
						pos			= $slice.data('position');
					$slice.removeClass("collapsed");
				...
				if( this.length ) {
					...
					return this.each(function() {
						...
						var $el 			= $(this),
							...
							$navNext		= $el.find('span.va-nav-next').css("position","fixed"),
							$navPrev = $el.find('span.va-nav-prev').css("position", "fixed");
							
						// each slice's height
						cache.sliceH = 50;//Math.ceil( settings.accordionH / settings.visibleSlices );
						...
						// click event
						$slices.children(".va-title").bind('click.vaccordion', function (e) {
						   // console.log(e.target.hasClass("va-title"));
